      ...
      <div className="w-4/5 h-full flex justify-between items-center">
        ...
        <div className="flex items-center">
          ...
          <div className="hidden flex items-center sm:block">
            <Button type="default" className="px-2 mx-2 lg:px-4 h-8 lg:h-9 custom-button" style={{ borderRadius: 5, fontWeight: 500 }}>Нэвтрэх</Button>
            <Button type="primary" className="px-2 mx-2 lg:px-4 h-8 lg:h-9 custom-button" style={{ borderRadius: 5, fontWeight: 500 }}>Бүртгүүлэх</Button>
          </div>
        </div>

        {/* Responsive sidebar */}
        <div className={clsx("fixed h-full w-screen top-0 right-0 2xl:hidden", {"hidden":!isSideMenuOpen})}>
          <section className="h-screen max-w-full bg-[#FFFFFF] flex flex-col absolute top-0 left-0 px-8 z-50 items-center">
            <div className="w-64 sm:w-80 h-16 sm:h-20 py-4 my-2 flex justify-between">
            ...
            <div className="block flex flex-col items-center sm:hidden">
              <Button type="text" icon={<MdLogin className="text-lg"/>} className="w-64 sm:w-80 flex justify-start py-5 md:py-6 text-[#8E8E93] text-sm custom-button">Нэвтрэх</Button>
              <Button type="text" icon={<MdOutlinePersonOutline className="text-lg"/>} className="w-64 sm:w-80 flex justify-start py-5 md:py-6 text-[#8E8E93] text-sm custom-button">Бүртгүүлэх</Button>
            </div>
            <div className="flex flex-col items-center">
              {navLinks.map((d, i)=>(
                <Button key={i} type="text" href={d.href} className="w-64 sm:w-80 py-5 md:py-6 flex justify-start text-[#8E8E93] text-sm custom-button" onClick={() => setMenu(false)}>{d.label}</Button>
              ))}
            </div>
          </section>
