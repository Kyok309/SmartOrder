      ...
      <div className="w-4/5 h-full flex justify-between items-center">
        ...
        <div className={clsx("fixed h-full w-screen top-0 right-0", {"hidden":!isSideMenuOpen})}>
          <section className="h-screen max-w-full bg-[#FFFFFF] flex flex-col absolute top-0 left-0 px-8 z-50 items-center">
            <div className="w-64 sm:w-80 h-20 py-4 flex justify-between">
            ...
            <div className="flex flex-col items-center">
              {navLinks.map((d, i)=>(
                <Button key={i} type="text" href={d.href} className="w-64 sm:w-80 py-6 flex justify-start text-[#8E8E93]" onClick={() => setMenu(false)}>{d.label}</Button>
              ))}
            </div>
          </section>
